...
because the function cannot be replaced by a layer. In such a case, you
can annotate the function with the `use_kernel_func_from_hub` function:
...
(singleton) that calls the function itself in its forward method. So, the
'function module'

**Note:** for kernelization to see the function, it must be a member of
another `torch.nn.Module` that is past of the model. For example:
...
If the layer is stateless (it does not use member variables _or_ it was
