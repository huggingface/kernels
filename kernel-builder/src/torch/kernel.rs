...
fn render_kernel_component_cpu(
    ...
    env.get_template("torch/cpu/kernel.cmake")
...
fn render_kernel_component_cuda(
    ...
    env.get_template("torch/cuda/kernel.cmake")
        .wrap_err("Cannot get kernel template")?
        .render_to_write(
            context! {
...
fn render_kernel_component_metal(
    ...
    env.get_template("torch/metal/kernel.cmake")
        .wrap_err("Cannot get kernel template")?
        .render_to_write(
            context! {
                cxx_flags => kernel.cxx_flags().map(|flags| flags.join(";")),
...
fn render_kernel_component_xpu(
    ...
    env.get_template("torch/xpu/kernel.cmake")
        .wrap_err("Cannot get kernel template")?
        .render_to_write(
            context! {
                cxx_flags => kernel.cxx_flags().map(|flags| flags.join(";")),
