...
import kernels
from kernels.minver import _installed_version, _warn_if_below_minver
from kernels.utils import _import_from_path, _loaded_kernels
...
def test_no_warning_when_minver_met(tmp_path, recwarn, minver):
    ...
    _warn_if_below_minver(metadata, variant_dir)
...
@pytest.mark.parametrize("minver", [42, "", "not-a-version"])
def test_invalid_minver_is_ignored(tmp_path, recwarn, minver):
    # A `kernels-minver` that cannot be parsed (e.g. written by a future
    # version that changed the format) must not fail the load.
    variant_dir = _write_variant(tmp_path, minver)
    metadata = Metadata.read_from_file(variant_dir / "metadata.json")
    _warn_if_below_minver(metadata, variant_dir)
    assert len(recwarn) == 0
