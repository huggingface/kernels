...
impl Digest {
    pub fn hash_variant(
        ...
        for entry in variant_files(variant_path) {
            let (entry, _) = entry.wrap_err("Failed to read directory entry for hashing")?;

            // Metadata file hash cannot be computed, since it stores hashes.
            // It is protected by the detached signature.
            if entry.depth() == 1
                && matches!(
                    entry.file_name().to_str(),
                    Some("metadata.json" | "metadata.json.sigstore")
                )
            {
                continue;
            }
...
mod tests {
    ...
    /// Variants in the Hub cache consist of symlinks into the blob store.
    #[cfg(unix)]
    #[test]
    fn hash_variant_follows_symlinks() -> Result<()> {
        let dir = TempDir::new()?;
        ...
        fs::write(blobs.join("blob"), b"hello world")?;
        std::os::unix::fs::symlink(blobs.join("blob"), variant_path.join("_extension.so"))?;

        let digest = Digest::hash_variant(DigestAlgorithm::SHA256, &variant_path)?;
