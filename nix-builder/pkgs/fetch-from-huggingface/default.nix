...

{
  ...
  hash,
  # Derivation name, defaults to `<owner>-<repo>-<revision>`.
  name ? null,
}:

stdenvNoCC.mkDerivation {
  name = lib.strings.sanitizeDerivationName (
    if name != null then name else "${owner}-${repo}-${revision}"
  );
  ...
  installPhase = ''
    ...
    python ${./download.py} \
      ${lib.escapeShellArg "${owner}/${repo}"} \
      ${lib.escapeShellArg type} \
      ${lib.escapeShellArg revision} \
      "$out"

    # Avoid anymetadata, which could make the output unreproducible.
    rm -rf "$out/.cache"
